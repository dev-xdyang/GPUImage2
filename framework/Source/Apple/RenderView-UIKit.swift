    ...
    public class RenderView: UIView, ImageConsumer {
        ...
        private lazy var displayShader: ShaderProgram = {
            sharedImageProcessingContext.passthroughShader
        }()
        ...
        func createDisplayFramebuffer() {
            ...
            sharedImageProcessingContext.context.renderbufferStorage(Int(GL_RENDERBUFFER), from: layer as! CAEAGLLayer)
        ...
        public func newFramebufferAvailable(_ framebuffer: Framebuffer, fromSourceIndex _: UInt) {
            ...
            activateDisplayFramebuffer()

            clearFramebufferWithColor(backgroundRenderColor)

            let scaledVertices = fillMode.transformVertices(verticallyInvertedImageVertices, fromInputSize: framebuffer.sizeForTargetOrientation(orientation), toFitSize: backingSize)
            renderQuadWithShader(displayShader, vertices: scaledVertices, inputTextures: [framebuffer.texturePropertiesForTargetOrientation(orientation)])
            framebuffer.unlock()
