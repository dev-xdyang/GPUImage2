            ...
            if let sampleBuffer = videoTrackOutput.copyNextSampleBuffer() {
                if playAtActualSpeed {
                    ...
                    if frameTimeDifference > actualTimeDifference {
                        usleep(UInt32(round(1_000_000.0 * (frameTimeDifference - actualTimeDifference))))
                    }
...
//         chrominanceFramebuffer.cache = sharedImageProcessingContext.framebufferCache
        ...
        convertYUVToRGB(shader: yuvConversionShader, luminanceFramebuffer: luminanceFramebuffer, chrominanceFramebuffer: chrominanceFramebuffer, resultFramebuffer: movieFramebuffer, colorConversionMatrix: conversionMatrix)
