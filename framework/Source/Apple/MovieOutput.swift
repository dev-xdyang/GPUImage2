...
public class MovieOutput: ImageConsumer, AudioEncodingTarget {
    ...
    public func startRecording(transform: CGAffineTransform? = nil) {
        ...
        sharedImageProcessingContext.runOperationSynchronously {
            self.isRecording = self.assetWriter.startWriting()
                ...
                _ = CVOpenGLESTextureCacheCreateTextureFromImage(kCFAllocatorDefault, sharedImageProcessingContext.coreVideoTextureCache, self.pixelBuffer!, nil, GLenum(GL_TEXTURE_2D), GL_RGBA, bufferSize.width, bufferSize.height, GLenum(GL_BGRA), GLenum(GL_UNSIGNED_BYTE), 0, &cachedTextureRef)
    ...
    func renderIntoPixelBuffer(_ pixelBuffer: CVPixelBuffer, framebuffer: Framebuffer) {
        #if os(iOS)
            if !sharedImageProcessingContext.supportsTextureCaches() {
                renderFramebuffer = sharedImageProcessingContext.framebufferCache.requestFramebufferWithProperties(orientation: framebuffer.orientation, size: GLSize(size))
        ...
        #else
            let renderFramebuffer = sharedImageProcessingContext.framebufferCache.requestFramebufferWithProperties(orientation: framebuffer.orientation, size: GLSize(size))
            renderFramebuffer.lock()
...
public extension Timestamp {
    init(_ time: CMTime) {
        ...
        flags = TimestampFlags(rawValue: time.flags.rawValue)
        epoch = time.epoch
    }
